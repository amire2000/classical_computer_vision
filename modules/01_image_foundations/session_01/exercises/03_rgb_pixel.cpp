...
int main() {
  ...
  // TODO: Compute the first channel index for pixel (row, column).
  const std::size_t first_channel = 0;
  const int channel_sum = image[first_channel] + image[first_channel + 1] +
                          image[first_channel + 2];

  assert(channels == 3);
  std::cout << "pixel(" << row << ", " << column << ") in width " << width
            << " has RGB channel sum = " << channel_sum << "\n";
